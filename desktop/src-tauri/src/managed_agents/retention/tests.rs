...
/// Two ids at an equal timestamp, ordered as the relay orders them.
fn lower_and_higher_ids() -> (String, String) {
    ("0".repeat(64), "f".repeat(64))
}

#[test]
fn test_equal_second_lower_id_inbound_replaces_non_pending_row() {
    // The relay resolves an equal-`created_at` collision by lowest event id, so
    // a lower-id inbound event IS the relay's head. A local cache that skipped
    // it would hold a head the relay disagrees with, and every later
    // disk-vs-head compare would inherit that error.
    let conn = test_db();
...
#[test]
fn test_equal_second_higher_id_inbound_loses_to_non_pending_row() {
    // The mirror direction: the retained row is the relay's head, so a
    // higher-id inbound event at the same second must not displace it.
    let conn = test_db();
    let (lower, higher) = lower_and_higher_ids();
    retain_event(
        &conn,
...
#[test]
fn test_equal_second_identical_id_is_a_noop() {
    // A re-received echo of the row we already hold: `>=` on the relay side,
    // so it loses, and nothing changes.
    let conn = test_db();
    let (lower, _) = lower_and_higher_ids();
    retain_event(
        &conn,
        &RetainedEvent {
            pending_sync: false,
    ...
    let echo = RetainedEvent {
        pending_sync: false,
        event_id: Some(lower),
        ..sample_event()
    };
    assert_eq!(
        retain_inbound_event(&conn, &echo).unwrap(),
        InboundOutcome::Skipped
    );
}
...
fn test_equal_second_lower_id_inbound_defers_to_pending_row() {
    ...
    let row = get_retained_event(&conn, 30175, "abc123", "test-persona")
        .unwrap()
        .unwrap();
    assert!(row.pending_sync, "local intent must survive");
    assert!(row.content.contains("Test"));
    assert_eq!(row.event_id, Some(higher));
}

#[test]
fn test_equal_second_unorderable_pair_leaves_retained_row_standing() {
    // A legacy row whose `raw_event` never verified has no ordering key. Both
    // directions of the missing-id case must skip rather than invent one: a
    // fabricated key could win a tie the row should lose.
    let conn = test_db();
    let (lower, _) = lower_and_higher_ids();

    for (retained_id, inbound_id) in [
        (None, Some(lower.clone())),
        (Some(lower.clone()), None),
        (None, None),
    ] {
        delete_retained_event(&conn, 30175, "abc123", "test-persona").unwrap();
        retain_event(
        ...
        let inbound = RetainedEvent {
            content: r#"{"display_name":"Remote"}"#.to_string(),
            pending_sync: false,
            event_id: inbound_id.clone(),
            ..sample_event()
        };
        assert_eq!(
            retain_inbound_event(&conn, &inbound).unwrap(),
            InboundOutcome::Skipped,
            "retained={retained_id:?} inbound={inbound_id:?}"
        );
